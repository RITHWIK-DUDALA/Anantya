...
export default function MovieStatusPage() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const res = await fetch(`${apiUrl}/api/verify/movie-status-login`, {
        method: 'POST',
  ...
  const navItems = [
    ...
    { label: "Movie Status", href: "/movies/status" }
  ];
          ...
          {userStatus ? (
            <div className="card" style={{ padding: '40px', background: 'rgba(255,255,255,0.03)', borderRadius: '24px', border: '1px solid rgba(255,255,255,0.08)', backdropFilter: 'blur(20px)', width: '100%', maxWidth: '600px', textAlign: 'center', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)' }}>
              <h2 style={{ color: 'var(--primary)', marginBottom: '20px', fontSize: '2rem', letterSpacing: '-0.5px' }}>
                Welcome, {userStatus.name ? userStatus.name.split(' ')[0] : 'Guest'}!
              </h2>
              ...
              <div style={{ padding: '40px 30px', margin: '20px 0', borderRadius: '16px', background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(255,255,255,0.05)', boxShadow: 'inset 0 2px 15px rgba(0,0,0,0.5)' }}>
                <p style={{ margin: '0 0 15px 0', color: '#888', fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '2px' }}>Ticket Status</p>
                
                {userStatus.status === 'verified' && (
                  <div style={{ animation: 'fadeIn 0.5s ease-out' }}>
                    <CircleCheckIcon size={72} color="var(--green)" />
                    <h3 style={{ color: 'var(--green)', margin: '20px 0 10px 0', fontSize: '2rem', textShadow: '0 0 20px rgba(0, 255, 136, 0.3)' }}>Confirmed!</h3>
                    <p style={{ color: '#aaa', margin: 0, fontSize: '1.1rem', lineHeight: 1.5 }}>Your movie ticket is confirmed. Enjoy the show!</p>
                  </div>
                )}
                ...
                {userStatus.status === 'pending_verification' && (
                  <div style={{ animation: 'fadeIn 0.5s ease-out' }}>
                    ...
                    <p style={{ color: '#aaa', margin: 0, fontSize: '1.1rem', lineHeight: 1.5 }}>Your ticket will be verified shortly. Please check back soon.</p>
                  </div>
                )}
                ...
                {userStatus.status === 'rejected' && (
                  <div style={{ animation: 'fadeIn 0.5s ease-out' }}>
                    ...
                    <p style={{ color: '#aaa', margin: '0 0 10px 0', fontSize: '1.1rem', lineHeight: 1.5 }}>
                      {userStatus.rejectionReason 
                        ? `Reason: ${userStatus.rejectionReason}. Please contact the coordinators.`
                        : 'We could not verify your transaction. Please contact the coordinators.'}
                    </p>
                  </div>
                )}
              </div>
              ...
              <div style={{ textAlign: 'left', background: 'rgba(255,255,255,0.02)', padding: '24px', borderRadius: '12px', marginBottom: '35px', border: '1px solid rgba(255,255,255,0.03)' }}>
                <p style={{ margin: '0 0 12px 0', fontSize: '1.05rem' }}><strong style={{ color: '#ccc', width: '150px', display: 'inline-block' }}>Movie Title:</strong> <span style={{ color: '#fff' }}>{userStatus.movieTitle}</span></p>
                <p style={{ margin: '0 0 12px 0', fontSize: '1.05rem' }}><strong style={{ color: '#ccc', width: '150px', display: 'inline-block' }}>Language:</strong> <span style={{ color: '#fff' }}>{userStatus.language}</span></p>
                <p style={{ margin: '0 0 12px 0', fontSize: '1.05rem' }}><strong style={{ color: '#ccc', width: '150px', display: 'inline-block' }}>Reporting Time:</strong> <span style={{ color: '#fff' }}>{userStatus.date} at {userStatus.time}</span></p>
                <p style={{ margin: '0 0 12px 0', fontSize: '1.05rem' }}><strong style={{ color: '#ccc', width: '150px', display: 'inline-block' }}>Seat(s):</strong> <span style={{ color: '#fff' }}>{userStatus.seats?.join(', ') || 'N/A'}</span></p>
                <p style={{ margin: '0 0 12px 0', fontSize: '1.05rem' }}><strong style={{ color: '#ccc', width: '150px', display: 'inline-block' }}>Payment Status:</strong> <span style={{ color: '#fff' }}>{userStatus.status === 'verified' ? 'Verified' : userStatus.status === 'pending_verification' ? 'Pending' : 'Rejected'}</span></p>
                <p style={{ margin: '0 0 12px 0', fontSize: '1.05rem' }}><strong style={{ color: '#ccc', width: '150px', display: 'inline-block' }}>Transaction ID:</strong> <span style={{ fontFamily: 'monospace', color: 'var(--primary)', letterSpacing: '1px' }}>{userStatus.paymentId}</span></p>
                <p style={{ margin: '0', fontSize: '1.05rem' }}><strong style={{ color: '#ccc', width: '150px', display: 'inline-block' }}>Booking ID:</strong> <span style={{ fontFamily: 'monospace', color: 'var(--primary)', letterSpacing: '1px' }}>{userStatus.bookingId}</span></p>
              </div>
            ...
            <div style={{ width: '100%', maxWidth: '440px', textAlign: 'center' }}>
              ...
              >
                <p style={{ color: '#888', textAlign: 'center', marginBottom: '30px', fontSize: '0.95rem', lineHeight: 1.6 }}>
                  Log in using the email you booked with and your 6-digit Booking Token.
                </p>
                ...
                <div className="form-group" style={{ marginBottom: '30px' }}>
                  <label htmlFor="login-token" style={{ color: '#ccc' }}>Booking Token</label>
                  <input 
                    id="login-token" 
                    type="text" 
                    required 
