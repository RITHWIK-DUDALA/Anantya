...
export function AsciiGlitchRipple({
  ...
  as = "a",
  className,
  dur = 1200, // Increased latency as requested
  delay = 0, // Added delay prop for staggered loading
  chars = '.,·-─~+:;=*π""┐┌┘┴┬╗╔╝╚╬╠╣╩╦║░▒▓█▄▀▌▐■!?&#$@0123456789*',
  preserveSpaces = true,
  spread = 1.0,
  ...props
  ...
  const stateRef = useRef({
    origTxt: children,
    origChars: typeof children === 'string' ? children.split("") : [],
    isAnim: false,
    cursorPos: 0,
    waves: [],
    animId: null,
  ...
  useEffect(() => {
    ...
    // Set cursor to center and trigger wave on load
    stateRef.current.cursorPos = Math.floor(stateRef.current.origTxt.length / 2);
    
    // Add a small delay to make the effect visible after render (using staggered delay)
  ...
  return (
    <Component
      ref={elRef}
      className={["cursor-pointer select-none relative inline-block transition-colors duration-200", className].filter(Boolean).join(" ")}
      style={{ cursor: 'pointer', userSelect: 'none', position: 'relative', display: 'inline-block', transition: 'color 0.2s', ...(props.style || {}) }}
      {...props}
    />
  );
}
